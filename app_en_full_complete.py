...
def read_any_file(f) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        st.error(f"Error reading file von {getattr(f, 'name', 'Datei')}: {e}")
        return None
...
def build_related_from_embeddings(
    ...
    if not pairs:
        return pd.DataFrame(columns=["Ziel", "Source", "Similarity"])
    return pd.DataFrame(pairs, columns=["Ziel", "Source", "Similarity"])
...
def read_any_file_cached(filename: str, raw: bytes) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        st.error(f"Error reading file von {filename or 'Datei'}: {e}")
        return None
...
1. **Interne Links finden**  
   - Auf Basis semantischer Similarity wird geprüft, ob thematisch verwandte Seiten bereits intern miteinander verlinkt sind.  
   - Das Tool schlägt zusätzlich sinnvolle interne Links vor und bewertet deren Potenzial mit einem **Link Potential-Score**.  
...
2. **Unpassende Links identifizieren**  
   ...
   - Grundlage ist die semantische Similarity sowie ein vereinfachter *PageRank-Waster-Ansatz* (Seiten mit vielen Outlinks, aber wenigen Inlinks).  
...
3. **Wertvollsten Links aus SEO-Sicht identifizieren (SEO-Potenziallinks)**  - öffnet sich automatisch nachdem Analysen 1 und 2 abgeschlossen wurden 
   - Ermittelt die **stärksten internen Linkgeber (Gems)** anhand des Link Potentials und listet **wertvolle, noch nicht vorhandene Content-Links** zu passenden Target URLs.  
...
- **Option 2: Related URLs**  
  ...
  - **URL 1**, **URL 2**, **Similarityswert (0–1)** (z. B. aus Screaming Frog *Massenexport → Inhalt → Semantisch ähnlich*).

- Zusätzlich erforderlich:  
  - **All Inlinks** (CSV/Excel, aus Screaming Frog: *Massenexport → Links → Alle Inlinks*) — enthält mindestens folgende Spalten: **Source/Source**, **Ziel/Destination**, optional **Linkposition/Link Position**. Weitere Spalten sind nicht erforderlich. **Tipp**: Lösche alle Linktypen, die keine Hyperlinks sind und die anderen Spalten vor dem Upload heraus, um eine geringere Dateigröße zu erzielen.
...
""")
    ...
    st.markdown("""
### ⚙️ Weighting (Link Potential)

Die Berechnung des Link Potentials basiert auf folgenden Faktoren:  
...
💡 **Interpretation des Link Potential-Scores in den Output-Dateien bei Analysen 1 und 3:**  
...
with st.sidebar:
    ...
    st.subheader("Weighting (Link Potential)")
    st.caption(
        "Das Link Potential gibt Aufschluss über die Lukrativität einer **URL** als Linkgeber. "
    )
    w_ils = st.slider(
        "Interner Link Score",
        0.0, 1.0, 0.30, 0.01,
        help=("Interner Link Score (Screaming Frog): PageRank-ähnliches Maß für interne Linkpopularität aus dem Crawl. "
              "Höherer ILS ⇒ Source kann mehr interne Linkkraft vererben.")
    )
    w_pr = st.slider(
        "PageRank-Horder-Score",
        0.0, 1.0, 0.35, 0.01,
        help=("Was ist ein PageRank-Horder?\n\n"
              ...
              "Solche URLs werden in der Link Potential-Kalkulation höher priorisiert.")
    )
    w_rd = st.slider(
        "Referring Domains",
        0.0, 1.0, 0.20, 0.01,
    ...
    max_related = st.number_input(
        ...
        help="Wie viele semantisch ähnliche Seiten sollen pro Target URL in die Analyse einbezogen werden?"
    )
    ...
    not_similar_threshold = st.slider(
        ...
        0.0, 1.0, 0.60, 0.01,
        help=("Interne Links gelten als schwach, wenn deren semantische Similarity ≤ diesem Wert liegt. "
...
mode = st.radio(
    ...
    horizontal=True,
    help="Either upload embeddings (app calculates 'Related URLs') or use existing 'Related URLs'.",
)
...
if mode == "URLs + Embeddings":
    st.write(
        "Lade eine Datei mit **URL** und **Embedding** (JSON-Array or Zahlen, getrennt durch Komma/Whitespace/;/|). "
...
- Source/Source: `quelle`, `source`, `from`, `origin`, `quell-url`
...
1) **Target URL**, 2) **Quell-URL**, 3) **Similarity (0–1)**.
...
if not run_clicked and not st.session_state.ready:
    st.info("Please upload all files and click **Let's Go** to start the analysis.")
    st.stop()
...
if run_clicked or st.session_state.ready:
    if mode == "URLs + Embeddings":
        ...
        if emb_df is not None and not emb_df.empty:
            ...
            - Nach L2-Normierung funktionieren Cosine-Similarityen wie gewohnt, sofern alle Embeddings aus **demselben Modell/Space** stammen.
            - Wenn Embeddings aus **verschiedenen Modellen** stammen, sind Similarityswerte nur bedingt vergleichbar.
            ...
            related_df = build_related_auto(
                ...
                prefer_backend=backend,   # "Exakt (NumPy)" or "Schnell (FAISS)"
...
if not have_all:
    st.error("Please provide all required tables.")
    st.stop()
...
if src_idx == -1 or dst_idx == -1:
    st.error("In 'All Inlinks' wurden die Spalten 'Source/Source' or 'Ziel/Destination' nicht gefunden.")
    st.stop()
...
rel_src_idx = find_column_index(rel_header, POSSIBLE_SOURCE)  # Source
...
if -1 in (rel_dst_idx, rel_src_idx, rel_sim_idx):
    if related_df.shape[1] >= 3:
        ...
        st.error("'Related URLs' braucht mindestens 3 Spalten (Ziel, Source, Similarity).")
        st.stop()
...
for _, r in related_df.iterrows():
    ...
    urlB = remember_original(r.iloc[rel_src_idx])   # Source
    try:
...
# Precompute "Link Potential" als reine Sourceigenschaft (ohne Similarity)
...
st.caption("Diese Analyse schlägt thematisch passende interne Verlinkungen vor, zeigt bestehende (Content-)Links und bewertet das Link Potential der Linkgeber.")
if not st.session_state.get("__gems_loading__", False):
    ...
    for i in range(1, int(max_related) + 1):
        cols += [
            f"Related URL {i}",
            f"Similarity {i}",
            f"Link von Related URL {i} auf Target URL bereits vorhanden?",
            f"Link von Related URL {i} auf Target URL present in content?",
            f"Link Potential Related URL {i}",
        ]
    ...
    for _, r in res1_df.iterrows():
        ...
        for i in range(1, max_i + 1):
            ...
            col_sim = f"Similarity {i}"
            col_any = f"Link von Related URL {i} auf Target URL bereits vorhanden?"
            col_con = f"Link von Related URL {i} auf Target URL present in content?"
            col_pot = f"Link Potential Related URL {i}"
    ...
    res1_view_long = pd.DataFrame(long_rows, columns=[
        "Target URL",
        "Related URL",
        "Similarity (Cosinus Similarity)",
        "Link von Related URL auf Target URL vorhanden?",
        "Link von Related URL auf Target URL present in content?",
        "Link Potential",
    ])
    
    # Optional: sortieren (erst Target URL, dann absteigend nach Similarity)
    if not res1_view_long.empty:
        res1_view_long = res1_view_long.sort_values(
            by=["Target URL", "Similarity (Cosinus Similarity)"],
    ...
    st.download_button(
        "Download 'Internal Linking Opportunities (Long-Format)' (CSV)",
        data=csv1,
    ...
    for _, r in related_df.iterrows():
        a = remember_original(r.iloc[rel_src_idx])  # Source
    ...
    # 3) Waster-Score (Source) + Klassifikation
    ...
    st.download_button(
        "Download 'Potential Links to Remove' (CSV)",
        data=csv2,
...
1) **Gems identifizieren**  
   Wir bewerten alle Seiten nach dem **Link Potential-Score** (Mix aus *Interner Link Score*, *PageRank-Horder*, *Backlinks*, *Ref. Domains*).  
   Die obersten *X %* (Slider **„Anteil starker Linkgeber“**) gelten als **Gems** = beste internen Linkgeber.

2) **Kandidaten-Ziele finden**  
   Für jede Gem-URL nehmen wir Target URLs aus **Analyse 1**, bei denen die Gem-URL als **Related URL** auftaucht **und** es **noch keinen Content-Link** vom Gem → Ziel gibt.  
   Die inhaltliche Nähe steuert die **Similaritysschwelle** in der Seitenleiste.

3) **Linkbedarf (PRIO) berechnen**  
   Jede Target URL erhält einen **PRIO-Wert** aus vier Signalen (Gewichte per Slider; Summe muss nicht 1 sein — wir normalisieren intern):
...
gem_pct = st.slider(
    ...
    1, 30, 10, step=1,
    help="Welche obersten X % nach Link Potential sollen als 'Gems' aka starke Linkgeber-URLs gelten?"
)
max_targets_per_gem = st.number_input(
    "Top-Ziele je Gem",
    min_value=1, max_value=50, value=10, step=1,
    help="Wie viele Target URLs pro Gem in der Output-Tabelle gezeigt werden."
)
...
gsc_up = st.file_uploader(
    ...
    help=(
    "Search Console Daten helfen bei der Priorisierung, welche Target URLs Gems verlinken sollten.\n"
...
with st.expander("Reihenfolge der Empfehlungen - *OPTIONAL*", expanded=False):
    st.caption(
        "Hier legst du fest, in welcher Reihenfolge die Target URLs pro Gem angezeigt werden:\n"
...
def deficit_weighted_for(target: str) -> float:
    ...
    if not isinstance(res1_df, pd.DataFrame):
        return 0.0
    row = res1_df.loc[res1_df["Target URL"] == target]
    if row.empty:
        return 0.0
    ...
    while True:
        ...
        cont_new = f"Link von Related URL {i} auf Target URL present in content?"
...
# --------------------------
# Gems bestimmen (aus Link Potential)
# --------------------------
...
if isinstance(res1_df, pd.DataFrame) and not res1_df.empty:
    for _, row in res1_df.iterrows():
        u = normalize_url(row["Target URL"])
        if not u:
            continue
...
try:
    ...
    sim_col        = lambda i: f"Similarity {i}"
    ...
    for _, row in res1_df.iterrows():
        target = normalize_url(row["Target URL"])
        if not target:
            continue
        i = 1
    ...
    if gem_rows:
        ...
        for gem, target, simv, prio_t, sortv in gem_rows:
            long_rows.append([
                ...
                round(pot_for(gem), 3),     # Link Potential (Quell-URL)
                disp(target),               # Target URL - Anzeigeform
