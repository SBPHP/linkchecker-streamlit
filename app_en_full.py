...
def read_any_file(f) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        st.error(f"Error while reading von {getattr(f, 'name', 'Datei')}: {e}")
        return None
...
def read_any_file_cached(filename: str, raw: bytes) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        st.error(f"Error while reading von {filename or 'Datei'}: {e}")
        return None
...
if not run_clicked and not st.session_state.ready:
    st.info("Please upload all required files and click **Let's Go** to start the analyses.")
    st.stop()
...
if src_idx == -1 or dst_idx == -1:
    st.error("In 'All Inlinks' wurden die Spalten 'Source/Source' oder 'Ziel/Destination' nicht gefunden.")
    st.stop()
...
st.markdown("#### Linkbedarf-Weighting für Zielseiten")
...
if not math.isclose(eff_sum, 1.0, rel_tol=1e-3, abs_tol=1e-3):
    st.caption(f"ℹ️ Aktuelle PRIO-Weightings-Summe: {eff_sum:.2f}. (kein Problem, wenn > 1 oder < 1, wird intern normalisiert)")
